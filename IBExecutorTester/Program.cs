    ...
    class Program
    {
        ...
        private static async Task Do(Dictionary<string, object> brokerClientConfig, string mongoDBHost, int mongoDBPort, string mongoDBName)
        {
            try
            {
                ...
                IBrokerClient brokerClient = await BrokerClient.SetupBrokerClient(IBrokerClientType.Trading, tradingExecutorRunner, brokerClientConfig, mongoDBServer, convertConnector, mdConnector, stopRequestedCts.Token, false);
        ...
        {
            int order1Id = await orderExecutor.PlaceTrailingStopOrder(EURUSD, BUY, 20000, 0.001, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
            Console.WriteLine("TrailingStopOrder1: {0} ({1})", order1Id > 0 ? "SUCCESS" : "FAILED", order1Id);
            Thread.Sleep(1000);

            int order2Id = await orderExecutor.PlaceTrailingStopOrder(EURUSD, SELL, 20000, 0.001, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
        ...
        {
            int order1Id = await orderExecutor.PlaceTrailingMarketIfTouchedOrder(EURUSD, BUY, 20000, 0.001, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
            Console.WriteLine("TrailingMarketIfTouchedOrder1: {0} ({1})", order1Id > 0 ? "SUCCESS" : "FAILED", order1Id);
            Thread.Sleep(1000);

            int order2Id = await orderExecutor.PlaceTrailingMarketIfTouchedOrder(EURUSD, SELL, 20000, 0.001, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
        ...
        {
            int order1Id = await orderExecutor.PlaceMarketOrder(EURUSD, SELL, 20000, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
            Console.WriteLine("MarketOrder1: {0} ({1})", order1Id > 0 ? "SUCCESS" : "FAILED", order1Id);
            Thread.Sleep(1000);

            int order2Id = await orderExecutor.PlaceMarketOrder(EURCHF, SELL, 20000, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
        ...
        {
            int orderId = await orderExecutor.PlaceLimitOrder(EURUSD, BUY, 20000, 1.115, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
        ...
        {
            int orderId = await orderExecutor.PlaceStopOrder(EURUSD, SELL, 20000, 1.115, DAY, "IBExecutorTester", ct: stopRequestedCts.Token);
