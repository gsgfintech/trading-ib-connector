...
namespace Net.Teirlinck.FX.InteractiveBrokersAPI.Executor
{
    public class IBPositionsExecutor : IPositionExecutor
    {
        ...
        public async Task<(bool Success, string Message)> RequestAccountsDetails(IEnumerable<string> accountNames)
        {
            if (accountNames.IsNullOrEmpty())
                return (false, $"Unable to request accounts details: {nameof(accountNames)} is null or empty");

            if (isRequestingAccountDetails)
                return (false, "Unable to request accounts details: another request is already in progress. Try again later");
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    foreach (var accountName in accountNames)
                    {
                        ...
                        received = received && accountDetailsReceivedEvent.WaitOne(TimeSpan.FromSeconds(3));
                    ...
                    if (received)
                        return (true, "Received accounts details");
                    else
                        return (false, "Didn't receive any account details in the specified timeframe of 3 seconds");
                }
